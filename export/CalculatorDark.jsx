...
export default function CalculatorDark({ onSend }) {
  ...
  // ── Estilos ───────────────────────────────────────────────────────────────
  const card = { position: 'relative', borderRadius: 28, padding: 24, background: 'rgba(255,255,255,.022)', backdropFilter: 'blur(16px) saturate(140%)', WebkitBackdropFilter: 'blur(16px) saturate(140%)', border: '1px solid rgba(255,255,255,.1)', boxShadow: '0 24px 60px rgba(2,8,30,.38), inset 0 1px 0 rgba(255,255,255,.16)' }
  const panel = bg => ({ borderRadius: 18, padding: '15px 16px', background: bg, border: '1px solid rgba(255,255,255,.07)' })
  ...
  return (
    <div style={{ width: '100%', maxWidth: 420, fontFamily: "'Space Grotesk',system-ui,sans-serif" }}>
      ...
      <div style={card}>
        ...
        {/* TU ENVÍAS */}
        <div style={panel('rgba(8,16,40,.25)')}>
        ...
        {/* DESTINATARIO RECIBE */}
        <div style={panel('rgba(56,189,248,.05)')}>
